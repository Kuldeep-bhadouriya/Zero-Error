...
function StatCard({ 
  ...
  return (
    <motion.div
      ...
      whileHover={{ y: -5 }}
    >
      <GlassCard className="relative overflow-hidden p-6 h-full group border-l-4" style={{ borderColor: `${colorMap[color].split(' ')[0].replace('text-', '')}` }}>
        <div className={`absolute right-0 top-0 w-32 h-32 bg-gradient-to-br ${gradientMap[color]} blur-3xl rounded-full opacity-20 group-hover:opacity-40 transition-all duration-500`} />
        
        <div className="flex flex-col h-full justify-between relative z-10">
          <div className="flex items-start justify-between mb-4">
            <div className={`p-0`}>
              <Icon className={`w-8 h-8 ${colorMap[color]}`} strokeWidth={1.5} />
            </div>
            {subValue && (
              <Badge variant="outline" className="bg-white/5 text-[10px] font-medium border-white/10 text-gray-400 tracking-wider">
                {subValue}
              </Badge>
            )}
          </div>
          ...
          <div>
            <p className="text-sm font-medium text-gray-400 mb-1 uppercase tracking-wide">{label}</p>
    ...
    <div className="space-y-8 pb-10">
      ...
      <div className="relative z-10">
        ...
        >
          <div>
            ...
            >
              <Badge variant="outline" className="border-red-500/30 text-red-500 bg-red-500/10 px-3 py-1">
            ...
            <p className="text-gray-400 mt-2 text-lg">Your gaming journey continues here.</p>
          </div>
        </motion.div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 space-y-8">
          ...
          >
            <GlassCard className="p-6 h-full border-blue-500/10" variant="intense">
              <div className="flex items-center gap-3 mb-6">
                <div className="p-2.5 rounded-xl bg-blue-500/10 text-blue-400">
                  <Rocket className="w-6 h-6" />
                </div>
                <div>
                  <h3 className="text-lg font-bold text-white">Earning Opportunities</h3>
                  <p className="text-xs text-gray-500">Boost your stats</p>
                </div>
              </div>
              ...
              <div className="space-y-4">
                {[
                  ...
                  <motion.div 
                    ...
                    className="flex items-center justify-between p-3 rounded-xl border border-white/5 transition-all cursor-pointer group"
                  >
                    ...
                    <span className="text-xs font-bold text-emerald-400 bg-emerald-400/10 px-2.5 py-1 rounded border border-emerald-400/20 shadow-[0_0_10px_rgba(52,211,153,0.1)]">
                      {item.value}
                    </span>
                  </motion.div>
                ))}
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.98 }}
                className="w-full mt-6 py-3 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-medium text-sm shadow-lg shadow-blue-500/20 hover:shadow-blue-500/30 transition-all"
              >
